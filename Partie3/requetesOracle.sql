/*Requêtes demandées pour l'étape 3 du projet, version Oracle */
...
SELECT DISTINCT nom_bouteille
FROM Bouteille
WHERE REGEXP_LIKE (nom_bouteille, '^(Château|Domaine).*');
...
-- Requête 4 : Elle permet d'afficher les vins blanc qui ne portent pas l'appellation VDQS, on utilise MINUS car EXCEPT n'existe pas en Oracle.
